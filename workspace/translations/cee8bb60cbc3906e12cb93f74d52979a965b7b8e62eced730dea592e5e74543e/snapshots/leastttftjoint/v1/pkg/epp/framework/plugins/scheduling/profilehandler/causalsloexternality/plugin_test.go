...
// TestAbortedRequestFoldsATruncatedOutputLength pins degradation D9, which is a real
// deviation from the specification rather than a design choice.
//
// The spec requires the per-class N_out mean to be updated ONLY on requests that actually
// completed, because a truncated count drags the estimate down. This hook cannot honour that:
// it carries no termination state, and the server FORCES a terminal call with EndOfStream
// true for every request that picked a pod but never completed -- error, client disconnect,
// or panic (pkg/epp/handlers/server.go:373-379). The flag that separates the two,
// reqCtx.ResponseComplete, is director-internal and is already set before the hook runs on
// the normal paths (server.go:537-540, :608-610).
//
// So the aborted request below is byte-for-byte indistinguishable from the completed one in
// TestResponseBodyAdvancesResidentAndFeedsNOut, and its truncated count IS folded. The test
// asserts that, rather than asserting the behaviour the spec wants, so the deviation is
// recorded where it happens instead of only in prose. What the port owes here is the counter,
// which is checked below.
//
// BIAS: N_out understated => residents look closer to finishing => less value at risk =>
// externality UNDER-counted => toward LOCAL, the same direction as D2b and D2c.
func TestAbortedRequestFoldsATruncatedOutputLength(t *testing.T) {
	handler, _ := buildArm(t, focalConfig())
	decode := testEndpoint("decode-1", bylabel.RoleDecode, "H100_SXM_80GB", nil)
	request := testRequest("r-abort", 4000)
	...
	before := readCounter(t, outputLengthFoldedCount, handler.typedName.Name, HandlerPluginType)

	// Two chunks, then the forced terminal call the abort path produces. Its Usage still
	// carries the last value seen, which for a disconnect is a PARTIAL output length.
	handler.ResponseBody(context.Background(), request,
		&fwkrc.Response{RequestID: "r-abort", Usage: fwkrh.Usage{CompletionTokens: 1}}, nil)
	handler.ResponseBody(context.Background(), request,
		&fwkrc.Response{RequestID: "r-abort", Usage: fwkrh.Usage{CompletionTokens: 12}}, nil)
	handler.ResponseBody(context.Background(), request,
		&fwkrc.Response{RequestID: "r-abort", EndOfStream: true, Usage: fwkrh.Usage{CompletionTokens: 12}}, nil)

	// The truncated count is folded. This is the deviation, asserted rather than hidden.
	if got := handler.policy.nHatFor(sloClassStandard); got != 12 {
		t.Errorf("nHat after an aborted request = %g, want the truncated 12: the hook cannot "+
			"distinguish an abort, so it folds; if this now differs, a termination signal "+
			"appeared upstream and D9 should be revisited", got)
	}
	if got := handler.table.size(); got != 0 {
		t.Errorf("the entry must still be released on the terminal call, got %d", got)
	}

	// The counter is what the port owes for an unavoidable degradation: the fold is
	// acceptable, the silence is the defect.
	if after := readCounter(t, outputLengthFoldedCount, handler.typedName.Name, HandlerPluginType); after != before+1 {
		t.Errorf("output-length fold counter = %g, want %g: without it the truncated folds are "+
			"invisible in the goodput figure", after, before+1)
	}
}
