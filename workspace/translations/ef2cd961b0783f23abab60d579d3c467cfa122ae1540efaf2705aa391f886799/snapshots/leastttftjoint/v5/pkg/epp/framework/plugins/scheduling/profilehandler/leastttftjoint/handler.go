...
const HandlerPluginType = "least-ttft-joint-handler"
...
// (scheduler.go:78), so one unfiltered profile sees BOTH pools, which is what lets a
...
// (configloader.go:275-284 errors with "multiple profile handlers found"), so this arm's
...
//
// THE SAME RULE SEPARATES THE TWO ARMS, and it is the reason the comparison is run as two
// scenarios rather than one. causal-slo-externality-handler is also a ProfileHandler, so
// declaring both arms in ONE plugins list is a startup error, not a silent A/B. Each arm
// ships its own scenario overlay and runs in its own EPP; both binaries carry both
// registrations, and the overlay decides which one is instantiated. A reviewer checking
// that the arms cannot interfere at runtime should check exactly this: they never share a
// process, so they never share a shadow table, a Policy, or a decision.
type Handler struct {
...
func HandlerFactory(name string, rawParameters *json.Decoder, handle fwkplugin.Handle) (fwkplugin.Plugin, error) {
	...
	h := &Handler{
		...
		// "<DataType>/<ProducerName>" (plugin/datakey.go:46-48), so an unbound read can
		...
		//
		// The two arms resolve their bindings INDEPENDENTLY, in separate processes, so
		// nothing in either binary can detect a divergence. The overlays are the only
		// place the equality is asserted, which is why Config.validate refuses an empty
		// name outright rather than defaulting one.
	...
	h.policy = newPolicy(cfg, metrics)
	return h, nil
}
...
// Pick selects which profiles to run. scheduler.go:63-92 calls it in a loop until it
...
// reqCtx.TargetEndpoint (director.go:461-472). Leaving both picks in the primary profile
...
// SchedulingResult (director.go:474).
//
...
// The stock disagg handler does the same (disagg_profile_handler.go:408).
func (h *Handler) PreRequest(ctx context.Context, request *fwksched.InferenceRequest,
	...
	if prefill == nil || len(prefill.TargetEndpoints) == 0 {
		return // local placement: no prefill header, by design
	}
...
// goroutine (director.go:541-552) while the final chunk runs on the request goroutine
...
func (h *Handler) ResponseBody(ctx context.Context, request *fwksched.InferenceRequest,
	...
	// TERMINAL CHUNK. Only a COMPLETED request yields a usable realized output length:
	// one that terminated early carries a truncated count that would drag the per-class
	// mean down. EndOfStream is the only completion signal the interface offers -- it
	// carries no error or termination state (requestcontrol/plugins.go:53-65) -- which
	// is precisely why the TTL sweep exists for the requests that never reach here.
...
//
// NamespacedName.String() is the identity the in-tree per-endpoint index also uses
// (inflightload/producer.go:364), so the shadow table keys agree with the rest of the
...
func endpointID(ep fwksched.Endpoint) string {
	...
	return ep.GetMetadata().NamespacedName.String()
}
