...
// producerEntry returns one producer's whole declaration -- its name and its parameters,
// decoded so that key order and whitespace cannot make two identical declarations compare
// unequal. The type is the lookup key, so it is not part of the returned value.
func producerEntry(plugins pluginsConfig, pluginType string) (map[string]any, bool) {
...
func TestOverlayIsCommonModeWithTheFocalArmOutsideTheObjective(t *testing.T) {
	for _, path := range overlayPaths(t) {
		...
		t.Run(filepath.Base(filepath.Dir(translation)), func(t *testing.T) {
			...
			// AND THE PRODUCERS THEMSELVES, not only the names that reference them.
			//
			// Signals above compares the producer NAMES each arm binds. That is not enough:
			// custom-plugins.yaml is a scalar string, so each treatment overlay REPLACES the
			// baseline's inner config and re-declares the producers itself. Two overlays can
			// bind the same tokenizedPromptProducerName while declaring token-producer with a
			// different vllm.url or modelName, or approx-prefix-cache-producer with different
			// parameters -- and every other assertion in this test would still pass while the
			// two arms tokenized different prompts or read differently-blocked prefixes.
			//
			// The agreement is maintained by hand across three files, so it is asserted here.
			for _, producer := range []string{tokenProducer, prefixProducer} {
				mineEntry, mineOK := producerEntry(mine, producer)
				theirsEntry, theirsOK := producerEntry(theirs, producer)
				if !mineOK || !theirsOK {
					t.Errorf("%s is declared in %d of the two overlays; both arms must declare it, "+
						"because the treatment string replaces the baseline's wholesale",
						producer, boolToInt(mineOK)+boolToInt(theirsOK))
					continue
				}
				if !reflect.DeepEqual(mineEntry, theirsEntry) {
					t.Errorf("the two shipped overlays declare %s differently:\n  comparator %+v\n"+
						"  focal      %+v\nthe arms would then read a differently-configured signal "+
						"under the same producer name, which is the hazard config.md section 5 "+
						"signal 11 exists to prevent", producer, mineEntry, theirsEntry)
				}
			}
