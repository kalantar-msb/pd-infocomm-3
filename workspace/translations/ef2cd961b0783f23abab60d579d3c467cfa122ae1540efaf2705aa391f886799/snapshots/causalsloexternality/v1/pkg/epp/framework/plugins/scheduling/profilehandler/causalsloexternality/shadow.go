...
//     queue, director.go:541-552) and read from the scheduling path, and the index
...
//     (plugin_state.go:51-54) and it is keyed per request.
//
...
// (config.md section 11). Without it, usage arrives only in the FINAL chunk, so
// StepsDone stays 0 for every request's whole lifetime and every remaining-steps
// estimate is wrong WHILE THE TABLE LOOKS PRESENT AND CORRECT. The plugin cannot
// detect that from inside -- a zero-StepsDone population is also what a freshly
// started fleet looks like -- so the shadowTableStepsAlwaysZero counter exists to make
// it visible over time rather than diagnosable in one sample.
...
func newShadowTable(cfg ShadowTable, metrics *pluginMetrics) *shadowTable {
	return &shadowTable{
		entries:          map[string]*residentEntry{},
...
// complete removes a resident that finished normally and reports its realized output
// length so the caller can fold it into the per-class mean.
//
// Only a COMPLETED request yields a usable output length: one that terminated without
// completing carries a truncated count that would drag the estimate down.
