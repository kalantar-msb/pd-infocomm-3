...
// TestRealTokenProducerPopulatesTheBodyPromptTokensReads closes the one gap the rest of this
// package cannot: every other test constructs request.Body.TokenizedPrompt directly in Go, so
// none of them can tell whether the REAL in-tree token-producer actually writes there.
//
// It matters because the failure would be silent and total. promptTokens reads
// request.Body.TokenizedPrompt (picker.go). If the producer instead published only to the
// data key this arm declares in Consumes(), promptTokens would return (0, false) on every
// request, the D8 decline path would run permanently, and the arm would never compute J --
// while the EPP looked healthy and kept reporting goodput. That is precisely the
// silent-fallback shape this port exists to prevent, so the contract is pinned rather than
// assumed.
//
// At this pin the producer stores on the BODY (tokenizer.go:383, `request.Body.TokenizedPrompt
// = tp`) and its Produces() declaration (:344-346) exists to order the DAG, not to carry the
// value. Upstream's own consumer reads the body the same way
// (disagg/prefix_based_pd_decider.go:167-176). The estimate backend is used here because it is
// the zero-config default and runs in-memory, so the test needs no tokenizer sidecar.
func TestRealTokenProducerPopulatesTheBodyPromptTokensReads(t *testing.T) {
	raw := json.NewDecoder(bytes.NewReader([]byte(`{"estimate":{}}`)))
	produced, err := tokenizer.PluginFactory(tokenizer.PluginType, raw, newFakeHandle())
	if err != nil {
		t.Fatalf("build the real %s: %v", tokenizer.PluginType, err)
	}
	producer, ok := produced.(fwkrc.DataProducer)
	if !ok {
		t.Fatalf("%s is not a DataProducer (%T)", tokenizer.PluginType, produced)
	}
	...
	request := &fwksched.InferenceRequest{
		...
		Body: &fwkrh.InferenceRequestBody{
			Completions: &fwkrh.CompletionsRequest{
				Prompt: fwkrh.Prompt{TokenIDs: []uint32{11, 22, 33, 44, 55}},
			},
		},
		Headers: map[string]string{},
	}

	// Before: absent, so the arm would decline.
	if _, tokenized := promptTokens(request); tokenized {
		t.Fatal("precondition: the body must start with no TokenizedPrompt")
	}

	if err := producer.Produce(context.Background(), request, nil); err != nil {
		t.Fatalf("Produce: %v", err)
	}

	// After: the producer wrote to the BODY, which is the field promptTokens reads.
	if request.Body.TokenizedPrompt == nil {
		t.Fatal("the token-producer did not populate request.Body.TokenizedPrompt, so promptTokens " +
			"would return (0,false) on every request and the arm would never compute J -- a silent " +
			"total fallback to the decline path")
	}
	got, tokenized := promptTokens(request)
	if !tokenized {
		t.Fatal("promptTokens still reports the prompt absent after the real producer ran")
	}
	if got != 5 {
		t.Errorf("promptTokens = %d, want 5 (the passed-through token IDs)", got)
	}
}
