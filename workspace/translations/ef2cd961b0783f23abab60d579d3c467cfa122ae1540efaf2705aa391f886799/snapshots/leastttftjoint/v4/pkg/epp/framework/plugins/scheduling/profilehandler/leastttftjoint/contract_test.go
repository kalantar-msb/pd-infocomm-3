...
// TestMetricNamesAreIdenticalToTheFocalArm enforces the operator's decision on metric naming,
// which went the opposite way from the obvious one.
//
// The names are the focal arm's, inherited VERBATIM, so this arm emits
// `causal_slo_externality_*` series while having no externality term. That is deliberate:
// config.md's D1 and D8 both require comparing the two arms' rates, the arms each run in their
// own EPP so the comparison is across runs, and identical family names plus differing
// plugin_type labels make each comparison a single query with a label selector. Renaming would
// split every one into two queries joined by hand, for no analytical gain.
//
// This test is the enforceable form of that decision. Renaming a family here -- which a future
// reader would plausibly do on the reasonable-looking grounds that the name is a misnomer --
// silently breaks those cross-arm queries without breaking anything a behavioural test would
// notice. See the header of metrics.go.
func TestMetricNamesAreIdenticalToTheFocalArm(t *testing.T) {
	...
	focal, mine := metricNames(t, string(focalRaw)), metricNames(t, string(mineRaw))
	if len(mine) != len(focal) {
		t.Fatalf("this arm declares %d metric families and the focal arm declares %d: the two must "+
			"be the same set, so a cross-arm query selecting on plugin_type sees both arms",
			len(mine), len(focal))
	}
	for i := range focal {
		if mine[i] != focal[i] {
			t.Errorf("metric %d differs: this arm %q, focal arm %q. The names are inherited "+
				"verbatim under config.md section 9.3 so that D1's estimator-regime comparison and "+
				"D8's decline-rate comparison are each ONE query with a plugin_type selector.",
				i, mine[i], focal[i])
		}
	}
}

// TestMetricsFileIsTheFocalArmsPlusDocumentationOnly pins how far metrics.go may drift.
//
// It is not under the byte-identity contract, because the operator's decision requires this
// arm to DOCUMENT the misnomer at the top of the file. So the permitted difference is exactly
// "added comment lines": every line of the focal arm's file must still be present here, in
// order, and every line this arm adds must be a comment or blank.
//
// That keeps the names, the Help strings, the label sets and the histogram buckets verbatim --
// which is what the cross-arm comparison depends on -- while allowing the explanation the
// decision requires.
func TestMetricsFileIsTheFocalArmsPlusDocumentationOnly(t *testing.T) {
	want := rewritePackageClause(readLines(t, filepath.Join(focalDir(t), "metrics.go")))
	got := readLines(t, "metrics.go")

	pos := 0
	for i, line := range got {
		if pos < len(want) && want[pos] == line {
			pos++
			continue
		}
		// A line not present in the focal file at this position must be added documentation.
		trimmed := strings.TrimSpace(line)
		if trimmed != "" && !strings.HasPrefix(trimmed, "//") {
			t.Errorf("metrics.go:%d is neither the focal arm's next line nor added documentation:\n  %q\n\n"+
				"This file may differ from ../causalsloexternality/metrics.go only by ADDED COMMENTS. "+
				"The metric names, Help strings, label sets and buckets are inherited verbatim so the "+
				"cross-arm D1 and D8 comparisons stay single queries.", i+1, line)
		}
	}
	if pos != len(want) {
		t.Errorf("only %d of the focal arm's %d metrics.go lines are present, in order: a line was "+
			"removed or rewritten rather than merely commented around", pos, len(want))
	}
}

// TestTheTwoArmsCannotBeInstantiatedTogether is the load-bearing safety argument for sharing
// metric family names, so it is asserted rather than left as prose.
//
// Sharing names WOULD collide: two packages declare distinct CounterVec pointers with the same
// fully-qualified names, and registerMetrics tolerates AlreadyRegisteredError only when the
// existing collector is pointer-identical to its own. The collision is safe purely because it
// is unreachable -- both arms' handlers implement fwksched.ProfileHandler, and
// buildSchedulerConfig permits exactly one across all instantiated plugins
// (configloader.go:275-284). Two arms in one EndpointPickerConfig is invalid before metrics
// matter, and registerMetrics runs at factory time rather than package init, so merely
// compiling both arms into one binary registers nothing.
//
// The test asserts BOTH halves: that the collision is real, and that the thing which makes it
// unreachable is true.
func TestTheTwoArmsCannotBeInstantiatedTogether(t *testing.T) {
	// Half one: both handlers are ProfileHandlers, which is what configloader rejects.
	...
	if _, ok := mine.(fwksched.ProfileHandler); !ok {
		t.Fatal("this arm's handler must be a ProfileHandler; the one-handler-per-config rule is " +
			"what makes shared metric names safe")
	}
	focal, err := causalsloexternality.HandlerFactory("focal", decoderFor(t, focalArmConfig()), newFakeHandleWithRegistry(prometheus.NewRegistry()))
	if err != nil {
		t.Fatalf("building the focal arm failed: %v", err)
	}
	if _, ok := focal.(fwksched.ProfileHandler); !ok {
		t.Fatal("the focal arm's handler must also be a ProfileHandler")
	}

	// Half two: the collision is real, so the rule above is doing actual work. Building the
	// focal arm against the registry this arm already populated must FAIL.
	_, err = causalsloexternality.HandlerFactory("focal", decoderFor(t, focalArmConfig()), newFakeHandleWithRegistry(registry))
	if err == nil {
		t.Error("registering the focal arm into a registry this arm already populated SUCCEEDED. " +
			"That means the metric families are no longer shared, and the reasoning in metrics.go's " +
			"header -- and the cross-arm single-query comparison it protects -- no longer holds.")
	}
}
