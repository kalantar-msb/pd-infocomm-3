...
func (r *Runner) registerInTreePlugins() {
	...
	//
	// REGISTERED Beta, and that is a deployment decision rather than a stability claim.
	...
	// paths below at :453 and :1125, with the flag at pkg/epp/server/options.go:239. That
	// flag is not expressible through the scenario overlay this arm is deployed from, so an
	// Alpha registration makes the arm fail to start rather than run -- and it would fail
	// only after the image is built and pushed, since nothing in the build or test gate
	// exercises it. The level gates that flag and nothing else reads it, so Beta buys a
	// startable arm at no cost in behaviour, and matches every in-tree profile handler and
	// picker this arm is modelled on (:611-615). These two types carry no upstream
	...
	//
	// Beta, like the arm above, and the deployment reason is the same -- but NOT for the
	// reason stated in that comment. `--allow-experimental-plugins` IS reachable from the
	// scenario overlay: router.epp.flags is read into $eppFlags at
	// config/charts/routerlib/templates/_deployment.yaml:8 and rendered as `--<key>=<value>`
	// args on the epp container at :118-120 (documented at values.yaml:29-32), and the
	// scenario's whole router block is passed through by
	// llm-d-benchmark's router-values template. So an Alpha registration would be
	// EXPRESSIBLE. It would still be the wrong choice here: it makes every arm's overlay
	// carry a required extra field whose omission blocks EPP startup after the image is
	// built and pushed, and nothing in the build or test gate exercises startup. The
	// stability level gates that flag and nothing else reads it, so Beta buys a startable
	// arm at no cost in behaviour and matches every in-tree profile handler and picker
	// these arms are modelled on (:611-615). Neither type carries an upstream
	// compatibility promise.
