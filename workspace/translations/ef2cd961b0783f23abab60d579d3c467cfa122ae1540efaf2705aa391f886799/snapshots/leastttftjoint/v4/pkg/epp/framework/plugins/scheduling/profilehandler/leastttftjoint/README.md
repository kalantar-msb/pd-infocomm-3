...
distinct decision-attribute key, metric family names **identical** to the focal arm's (see
Observability), and that the two arms can never be instantiated together — which is what makes
those shared names safe.
...
Ten metric families, all labelled `plugin_name` / `plugin_type`. The two that gate reading
any result at all are `causal_slo_externality_estimator_substitution_total` (D1) and
`causal_slo_externality_tokenization_unavailable_total` (D8).

**The metric names are the focal arm's, inherited verbatim, and they are a deliberate
misnomer here.** This arm emits `causal_slo_externality_*` series while having no externality
term at all — no value kernels, no `selfGood`, no capacity account. Anyone reading Prometheus
rather than this source will find that surprising, so, stated plainly:

> **The two arms are distinguished by the `plugin_name` and `plugin_type` labels, not by the
> metric name.** This arm's series arrive with `plugin_type="least-ttft-joint-handler"` or
> `"least-ttft-joint-picker"`.

Sharing the names is the right trade even though renaming looks tidier. D1 requires comparing
the two arms' estimator regimes and D8 requires comparing their tokenization-decline rates
before any result is read. Each arm is its own overlay and its own EPP, so those comparisons
happen *across runs* — and identical family names plus differing `plugin_type` labels make
each one a **single query with a label selector**. Renaming would split every one into two
queries joined by hand, for no analytical gain, since the labels already separate the arms.

`contract_test.go` enforces the naming both ways: `TestMetricNamesAreIdenticalToTheFocalArm`
fails if a family is renamed, and `TestMetricsFileIsTheFocalArmsPlusDocumentationOnly` allows
`metrics.go` to differ from the focal arm's by *added comments only*, so the names, Help
strings, label sets and histogram buckets stay verbatim.

### The collision, and one debugging trap

Shared names *would* collide: two packages declare distinct `CounterVec` pointers with the
same fully-qualified names, and `registerMetrics` tolerates `AlreadyRegisteredError` only when
the existing collector is pointer-identical to its own. It is safe purely because it is
**unreachable** — both arms' handlers implement `fwksched.ProfileHandler` and
...
matter, and `registerMetrics` runs at factory time rather than package init, so compiling both
arms into one binary registers nothing. `TestTheTwoArmsCannotBeInstantiatedTogether` asserts
both halves: that the collision is real, and that the rule making it unreachable holds.

**The trap:** `instantiatePlugins` runs at `configloader.go:147`, *before* the uniqueness check
at `:160`. So if both arms are ever configured together, the second arm's factory fails first
with a Prometheus duplicate-registration error, which **masks** the clear "multiple profile
handlers found" diagnosis. If you are debugging a metrics-registration failure at startup,
check whether the config declares two ProfileHandlers before investigating `metrics.go`.
