...
// TestNoSLOValueConfigReachesThisObjective is the STRUCTURAL proof of "what this arm does
// not read", as opposed to the declaration in Objective.ReadsSLOValueConfig or the
// validation that enforces the absence.
//
// Both of those say the arm should not read tau, V, the ablation switches, or the capacity
// account. Neither shows that no code path DOES. The objective reaches deep into the shared
// package -- admission contexts, remaining-steps estimates, the rollout guards -- and any
// one of those could read the SLO-value half without a single test noticing, because the
// shipped overlay omits it and an unread zero is indistinguishable from an unread value.
//
// So: run the objective twice over configurations that differ ONLY in that half, one of them
// filled with values chosen to be wildly wrong, and require the cost to be bit-identical.
// A path that read any of them would move.
//
// The fixture deliberately does not validate, which is what makes this possible: the real
// factory REFUSES the poisoned configuration, and that refusal is covered separately by
// TestConfigRejectsTheSLOValueHalf. The two tests are the two halves of the same property --
// nothing reads it, and nothing may set it.
func TestNoSLOValueConfigReachesThisObjective(t *testing.T) {
	poisoned := comparatorConfig()
	poisoned.V = 1e9
	poisoned.ActiveWorkload = workloadInteractive
	poisoned.WorkloadTargets = map[string]causalsloexternality.SLOTargets{
		workloadInteractive: {TauTTFTUs: 1, TauITLUs: 1, TauE2EUs: 1},
	}
	poisoned.Ablation = causalsloexternality.Ablation{
		NoExternality:     true,
		NoOwnGood:         true,
		OccupancyCapacity: true,
		Decomposed:        true,
	}
	poisoned.Capacity = causalsloexternality.Capacity{TauRefUs: 1, NomPrefillTokens: 1, ReferenceBatch: 1}

	ds := congestedDecode("default/d1", gpuH100)
	ps := idlePrefill("default/p1", gpuA100)

	plainEval := evalFor(t, comparatorConfig(), testInputLen, nil, ds, ps)
	poisonedEval := evalFor(t, poisoned, testInputLen, nil, ds, ps)

	for name, costs := range map[string][2]float64{
		"local": {
			Objective{}.Cost(plainEval, ds, nil),
			Objective{}.Cost(poisonedEval, ds, nil),
		},
		"disaggregated": {
			Objective{}.Cost(plainEval, ds, &ps),
			Objective{}.Cost(poisonedEval, ds, &ps),
		},
	} {
		if costs[0] != costs[1] {
			t.Errorf("%s cost moved when only the SLO-value half of the configuration changed: "+
				"%.6f against %.6f. Some path from this objective reads tau, V, an ablation "+
				"switch, or the capacity account, and this arm is specified to read none of them",
				name, costs[0], costs[1])
		}
	}

	// AND THE PROBE MUST HAVE TEETH. An invariance test passes trivially if the poisoning
	// is inert -- if the fields were never plumbed into the Policy at all, or the fixture
	// dropped them. Running the FOCAL objective over the SAME two fixtures must MOVE,
	// because its cost is V*(externality - ownGood) and both operands read that half.
	// Without this half of the test, a future refactor that stopped passing Config through
	// would make the assertion above vacuous and green.
	focal := causalsloexternality.FocalArm.Objective
	plainFocal, stopPlain, err := (causalsloexternality.EvalFixture{
		Config: focalConfig(), Objective: focal,
		Snapshots: []causalsloexternality.Snapshot{ds, ps},
		SLOClass:  sloClassStandard, InputLen: testInputLen,
	}).Build()
	if err != nil {
		t.Fatalf("build the focal control fixture: %v", err)
	}
	t.Cleanup(stopPlain)

	poisonedFocalConfig := focalConfig()
	poisonedFocalConfig.V = 1e9
	poisonedFocal, stopPoisoned, err := (causalsloexternality.EvalFixture{
		Config: poisonedFocalConfig, Objective: focal,
		Snapshots: []causalsloexternality.Snapshot{ds, ps},
		SLOClass:  sloClassStandard, InputLen: testInputLen,
	}).Build()
	if err != nil {
		t.Fatalf("build the poisoned focal control fixture: %v", err)
	}
	t.Cleanup(stopPoisoned)

	if focal.Cost(plainFocal, ds, nil) == focal.Cost(poisonedFocal, ds, nil) {
		t.Error("the control is inert: changing v did not move the FOCAL objective, so the " +
			"invariance assertion above proves nothing about this arm")
	}
}
