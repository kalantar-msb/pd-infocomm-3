...
- [Serving both experimental arms](#serving-both-experimental-arms)
...
The handler owns the state; the picker looks it up by `handlerPluginName`.

**Construction order comes from that reference, not from position in the plugins list.**
The loader topologically sorts a plugin DAG before constructing anything and builds
dependencies first, and the edge is supplied by the `pluginRef` tag on the picker's
`handlerPluginName` field, registered through `RegisterWithPluginDependencies`. So the two
entries may appear in either order. Removing the reference breaks **startup** rather than
ordering: both plugins then sit at in-degree 0 and their relative order comes from ranging a
Go map, so the picker is built first in most starts, its lookup returns nil, and the factory
errors rather than constructing a second, divergent copy of the shadow table. The handler is
nonetheless written first in the shipped configuration, for readability. See
`dependency_test.go`.
...
## Serving both experimental arms

The `Handler`, the `Picker`, and the decider in `Policy.decide` are shared by **two**
registered arms of the same experiment. They differ in one thing: the per-candidate cost
function, supplied as an `Objective` (`arm.go`).

| arm | package | handler type | picker type | objective |
|---|---|---|---|---|
| focal | this one | `causal-slo-externality-handler` | `causal-slo-externality-picker` | `V*(externality - ownGood)`, signed and unbounded |
| comparator | `../leastttftjoint` | `least-ttft-joint-handler` | `least-ttft-joint-picker` | the arriving request's projected TTFT, a microsecond latency |

The sharing is the point rather than a convenience. The comparator exists to attribute a
measured difference to the **mechanism** rather than to the **machinery**, so it must run the
same candidate set, the same estimators, and the same physics. Sharing one implementation is
stronger than maintaining two copies, because a copy can drift under an edit to one side.
An arm's package therefore contains no physics: everything its objective reads arrives
through `Eval`, which is a set of one-line delegations to `Policy`. If an arm needs a term
`Eval` does not expose, add the accessor there rather than re-deriving the term in the arm.

Two facts besides the cost function are declared per objective, because both are silent
failure surfaces:

- `ScorerFirstEnumeration` -- whether the inherited scorer's pick is moved to the front as a
  tie-break order. The focal arm does; the comparator does not. It changes behaviour only on
  an **exact** tie.
- `ReadsSLOValueConfig` -- whether the objective reads `v`, `ablation`, `activeWorkload`,
  `workloadTargets`, and `capacity`. See the note under [Configuration](#configuration).

**Only one arm runs per EPP.** Exactly one `ProfileHandler` is permitted per configuration,
so a config naming both handlers fails to start. Both types are registered in the binary and
the treatment overlay selects an arm by naming its two types, one overlay per arm.

A picker binding to the **other** arm's handler is refused at construction. Both arms are
the same Go type, so a type assertion cannot tell them apart, and the objective lives in the
`Policy` the handler owns -- an unchecked binding would run the other arm's rule while every
...
The metric collectors are shared and carry `plugin_name` and `plugin_type` labels, which is
how the two arms' D1 and D8 rates are compared. That comparison is a precondition for
reading any result, not a diagnostic.

**The SLO-value half of the parameters is arm-gated in both directions.** For an objective
that reads `v`, `ablation`, `activeWorkload`, `workloadTargets`, and `capacity` they are
required, because an unresolvable tau triple *flattens* the policy rather than loosening it.
For one that does not, they must be **absent**, and setting one is a startup error: these
keys are known to the struct, so `DisallowUnknownFields` accepts them, and a
populated-but-never-read field implies a consumer that does not exist. The parameter table
below is the focal arm's; the comparator's is the same table minus those five entries.
