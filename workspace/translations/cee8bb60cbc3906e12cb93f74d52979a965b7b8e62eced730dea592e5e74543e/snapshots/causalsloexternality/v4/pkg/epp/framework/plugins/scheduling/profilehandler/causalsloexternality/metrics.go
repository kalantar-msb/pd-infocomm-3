...
//   - realized output lengths folded into the N_out mean -- an unknown share are truncated
//     counts from aborted requests, which the response hook cannot distinguish (D9).
...
var (
	...
	// DEGRADATION D9's counter. The response hook carries no termination state
	// (pkg/epp/framework/interface/requestcontrol/plugins.go:76-86 records an upstream TODO
	// to add one), and the server FORCES a terminal call with EndOfStream true for any
	// request that picked a pod but never completed -- error, client disconnect, or panic
	// (pkg/epp/handlers/server.go:373-379). ResponseComplete, which separates the two, lives
	// on the director's request context and is set BEFORE the hook runs on the normal paths
	// (server.go:537-540, :608-610), so a plugin cannot tell a completion from an abort.
	//
	// Every terminal observation therefore folds its CompletionTokens into the per-class
	// N_out mean, and an aborted request contributes a TRUNCATED count. This counter makes
	// the total foldable population visible so the abort share can be reasoned about against
	// the reap count and the request total; it cannot attribute individual folds.
	outputLengthFoldedCount = prometheus.NewCounterVec(
		prometheus.CounterOpts{
			Subsystem: eppmetrics.LLMDRouterEndpointPickerSubsystem,
			Name:      "causal_slo_externality_output_length_folded_total",
			Help: metricsutil.HelpMsgWithStability(
				"Total realized output lengths folded into the per-class N_out mean, i.e. terminal "+
					"response-body observations. The hook carries no termination state and the server "+
					"forces a terminal call on abort, so an unknown share of these are TRUNCATED counts "+
					"from requests that never completed, which biases N_out DOWN and the externality "+
					"toward LOCAL. Read against the request total to bound that share.",
				compbasemetrics.ALPHA),
		},
		[]string{"plugin_name", "plugin_type"},
	)
...
func (m *pluginMetrics) recordOutputLengthFolded() {
	outputLengthFoldedCount.WithLabelValues(m.name, m.typ).Inc()
}
