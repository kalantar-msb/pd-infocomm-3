...
//
// IT ALSO NEVER CALLS startSweeper, so a fixture-built shadow table never reaps. TTL and
// sweep behaviour are therefore NOT covered by anything built here -- they belong to the
// shared package's own tests. The returned stop function is still safe to call, and must
// be called: shadowTable.stop is Once-guarded around closing the stop channel, so it is a
// no-op on a table whose sweeper never started.
