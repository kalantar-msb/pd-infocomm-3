...
func (h *Handler) ResponseBody(ctx context.Context, request *fwksched.InferenceRequest,
	...
	// from being silent. Declining to fold instead is worse, not more conservative: the mean
	// would never update, nHatFor would return its seed of 1 forever, and every
	// remaining-steps estimate would collapse.
	//
