...
func TestDecideEnumeratesTheFullCrossProduct(t *testing.T) {
	...
	//
	// THIS TEST GUARDS THE CROSS-PRODUCT SHAPE, NOT THE TIE-BREAK, and the distinction is
	// measured rather than asserted: with sortedByID deleted from decide's DECODE axis, this
	// test still passes 10/10 while TestTieBreakIsDeterministicOnBothCrossProductAxes fails
	// 10/10. It cannot detect that mutation for two independent reasons -- the fixture's inputs
	// are already in ascending ID order, and the expectation below MIRRORS the rule under test
	// by calling sortedByID itself, so the mutation is applied to both sides and cancels.
	//
	// That is the general hazard of an expectation recomputed from production's own ordering
	// rule: it follows production into a bug. The pinned test is the one carrying the tie-break
	// guarantee, because it asserts a CONCRETE winner and verifies the tie is exact first.
	//
	// KEEP BOTH, AND IF ONE MUST GO IT IS NOT THIS ONE'S GUARANTEE THAT IS DUPLICATED. The two
	// overlap in appearance only: this test checks that all D + D*P candidates are enumerated
	// and that the argmin is the minimum over the whole set; the pinned test checks how an exact
	// tie resolves. Deleting the pinned test on the grounds that this one "already covers
	// determinism" would remove the only real coverage of the tie-break.
