...
//
// A comment cannot enforce that, so verbatim_test.go does. It reads the focal arm's
...
// the focal arm's one-line summary must not be copied over. See estimatorRollforward's
// doc in config.go for the algebra and for why it bears on the attribution argument rather
// than only on this arm's accuracy.
//
