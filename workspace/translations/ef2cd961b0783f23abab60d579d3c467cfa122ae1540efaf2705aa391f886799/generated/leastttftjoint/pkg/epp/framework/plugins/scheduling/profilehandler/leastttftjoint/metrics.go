...
//
// The ten families below are `least_ttft_joint_*`, parallel to the focal arm's
// `causal_slo_externality_*`. Everything else about them is deliberately term-for-term
// identical: the same ten metrics, the same label sets, the same semantics, the same histogram
// buckets. Only the family name differs.
//
// THE REASON IS THAT THE FOCAL ARM'S NAMES WOULD BE A MISNOMER HERE, not that they would
// collide. Sharing them would have this arm emit `causal_slo_externality_*` series while it
// has no externality term at all -- no value kernels, no selfGood, no capacity account -- so
// anyone reading Prometheus rather than this source would be misled about what the series
// measure.
//
// A COLLISION IS NOT THE ARGUMENT, AND SAYING SO WOULD BE WRONG. It is unreachable in any
// valid configuration: registerMetrics runs at FACTORY time, not package init, so compiling
// both arms into one binary registers nothing; and instantiating both is already invalid
...
//
// Distinct names do, however, keep one failure mode legible. instantiatePlugins runs at
// configloader.go:147, BEFORE that uniqueness check at :160, so a config declaring both arms
// fails during instantiation. With distinct families it fails on the ProfileHandler rule with
// a clear message; with shared families the second arm's factory would fail first on a
// Prometheus duplicate registration, MASKING the real diagnosis.
//
// # WHAT THIS COSTS THE CROSS-ARM COMPARISON: ESSENTIALLY NOTHING
//
// config.md's D8 requires comparing the two arms' tokenization-decline RATES before any result
// is read, and D1 requires comparing their estimator regimes. Each arm is its own overlay and
// its own EPP, so those comparisons happen ACROSS RUNS.
//
// Distinct names do NOT force two separate queries. Because the families are term-for-term
// identical, a regex over __name__ recovers a single query, for example
//
//	{__name__=~".*_(least_ttft_joint|causal_slo_externality)_tokenization_unavailable_total"}
//
// and the plugin_name/plugin_type labels every collector carries then separate the arms within
// it. Keeping the metric name honest therefore costs a regex rather than a second query, which
// is why the misnomer was the deciding consideration.
