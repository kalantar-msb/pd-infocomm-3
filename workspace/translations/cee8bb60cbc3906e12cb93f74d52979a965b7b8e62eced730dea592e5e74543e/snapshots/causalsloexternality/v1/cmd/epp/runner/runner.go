...
func (r *Runner) registerInTreePlugins() {
	...
	// ValidatePluginStability refuses to start the EPP when any instantiated plugin is
	// Alpha and --allow-experimental-plugins is absent
	// (pkg/epp/framework/interface/plugin/registry.go:107-119). That flag is not
	// expressible through the scenario overlay this arm is deployed from, so an Alpha
	// registration makes the arm fail to start rather than run. The level gates only
	// that flag and nothing else reads it, so Beta buys a startable arm at no cost in
	// behaviour. These two types carry no upstream compatibility promise.
