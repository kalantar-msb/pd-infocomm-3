...
func loadOverlayPluginsAt(t *testing.T, path string) pluginsConfig {
	t.Helper()
	plugins, _ := loadOverlayAt(t, path)
	return plugins
}

// loadOverlayAt returns the inner plugin config AND the scenario name, which is a separate
// Criterion 5 requirement: llmdbenchmark deep-merges by scenario name, so a name that does not
// match the experiment's baseline deploys a SECOND scenario instead of merging into it -- both
// arms then run, neither as configured, and nothing errors.
...
func TestOverlayHandlerParametersDecodeAndValidate(t *testing.T) {
	forEachOverlay(t, func(t *testing.T, plugins pluginsConfig) {
		...
		if err := cfg.validate(focalObjective{}); err != nil {
...
// TestOverlayIdentityFieldsAreCorrect asserts the three fields the overlay carries that the
// other tests decode but never check. Each one fails SILENTLY when wrong, which is why they
// are worth pinning rather than eyeballing.
//
//   - scenario name: llmdbenchmark deep-merges by name, so a mismatch against the
//     experiment's baseline deploys a SECOND scenario instead of merging into it. Both arms
//     then run, neither as configured, and nothing errors.
//   - apiVersion: `inference.networking.x-k8s.io/v1alpha1` still loads but is registered only
//     as deprecatedSchemeGroupVersion (pkg/epp/config/loader/configloader.go:53) and logs a
//     deprecation. `llm-d.ai/v1alpha1` is the live group (apix/config/v1alpha1:15).
//   - kind: anything other than EndpointPickerConfig fails to decode against the scheme.
//
// The expected scenario name is read from the experiment's own baseline file rather than
// hardcoded, so the test cannot drift from the thing it is checking against.
func TestOverlayIdentityFieldsAreCorrect(t *testing.T) {
	want := expectedScenarioName(t)
	for _, path := range overlayPaths(t) {
		hash := filepath.Base(filepath.Dir(filepath.Dir(filepath.Dir(path))))
		t.Run(hash, func(t *testing.T) {
			plugins, name := loadOverlayAt(t, path)
			if name != want {
				t.Errorf("scenario name = %q, want %q from the experiment's baselines/baseline.yaml; "+
					"a mismatch makes llmdbenchmark deploy a second scenario instead of merging", name, want)
			}
			if plugins.APIVersion != "llm-d.ai/v1alpha1" {
				t.Errorf("apiVersion = %q, want \"llm-d.ai/v1alpha1\" (the live group; the "+
					"x-k8s.io spelling is registered only as deprecated)", plugins.APIVersion)
			}
			if plugins.Kind != "EndpointPickerConfig" {
				t.Errorf("kind = %q, want \"EndpointPickerConfig\"", plugins.Kind)
			}
		})
	}
}

// expectedScenarioName reads the scenario name from the experiment's baseline bundle, which is
// the authority llmdbenchmark merges against. SKIPS when the bundle is absent, for the same
// reason overlayPaths does: the plugin is testable independently of any one experiment.
func expectedScenarioName(t *testing.T) string {
	t.Helper()
	repoRoot, err := filepath.Abs(filepath.Join("..", "..", "..", "..", "..", "..", ".."))
	if err != nil {
		t.Fatalf("resolve repo root: %v", err)
	}
	baseline := filepath.Join(filepath.Dir(repoRoot), "baselines", "baseline.yaml")
	raw, err := os.ReadFile(baseline)
	if err != nil {
		t.Skipf("no experiment baseline at %s; skipping scenario-name check", baseline)
	}
	var parsed struct {
		Scenario []struct {
			Name string `json:"name"`
	...
	if err := yaml.Unmarshal(raw, &parsed); err != nil {
		t.Fatalf("parse %s: %v", baseline, err)
	}
	if len(parsed.Scenario) != 1 || parsed.Scenario[0].Name == "" {
		t.Fatalf("%s does not declare exactly one named scenario", baseline)
	}
	return parsed.Scenario[0].Name
}
