...
// PickerConfigParser is the FOCAL arm's ConfigParserFunc. It is
// NewPickerConfigParser(FocalArm) -- see arm.go.
var PickerConfigParser = NewPickerConfigParser(FocalArm)

// parsePickerParameters is the ConfigParserFunc body the loader uses to discover this
// picker's plugin dependencies BEFORE any plugin is constructed. It only parses; the
// real validation stays in the picker factory, which is the single place a bad
...
func parsePickerParameters(arm Arm, rawParameters *json.Decoder) (any, error) {
	parameters := pickerParameters{}
	if rawParameters != nil {
		if err := rawParameters.Decode(&parameters); err != nil {
			return nil, fmt.Errorf("failed to parse parameters of the %s plugin: %w", arm.PickerType, err)
		}
	}
...
// PickerFactory builds the FOCAL arm's picker. It is NewPickerFactory(FocalArm) -- see
// arm.go.
var PickerFactory = NewPickerFactory(FocalArm)

// newPicker builds one arm's picker and binds it to the handler that owns the state.
func newPicker(arm Arm, name string, rawParameters *json.Decoder, handle fwkplugin.Handle) (fwkplugin.Plugin, error) {
	parsed, err := parsePickerParameters(arm, rawParameters)
	if err != nil {
		return nil, err
	}
	parameters, ok := parsed.(pickerParameters)
	if !ok {
		return nil, fmt.Errorf("%s: unexpected parsed parameter type %T", arm.PickerType, parsed)
	}
	if parameters.HandlerPluginName == "" {
		return nil, fmt.Errorf("%s requires handlerPluginName: the argmin and the response hooks must "+
			"share ONE Policy and ONE shadow table, and the handler owns them", arm.PickerType)
	}
	...
	if raw == nil {
		return nil, fmt.Errorf("%s: handlerPluginName %q not found; it must name a %s in the same "+
			...
			arm.PickerType, parameters.HandlerPluginName, arm.HandlerType)
	}
	handler, ok := raw.(*Handler)
	if !ok {
		return nil, fmt.Errorf("%s: plugin %q is not a %s", arm.PickerType, parameters.HandlerPluginName, arm.HandlerType)
	}
	// THE CROSS-ARM BINDING GUARD, and it is not defensive boilerplate. Both registered
	// arms are the SAME Go type -- that is the whole point of the shared machinery -- so
	// the assertion above passes for either one, and a picker pointed at the other arm's
	// handler would construct successfully and then run the OTHER ARM'S OBJECTIVE, because
	// the objective lives in the Policy the handler owns. Every log line, every
	// plugin_type metric label, and the treatment overlay's own name would say this arm.
	// That is a silent arm swap: the measurement would be reported as a comparison
	// between two arms while both EPPs ran one of them.
	if handler.arm.HandlerType != arm.HandlerType {
		return nil, fmt.Errorf("%s: plugin %q is a %s, which owns a DIFFERENT arm's objective; "+
			"a %s must name a %s, or the argmin would silently run the other arm's rule while "+
			"every log line and metric label reported this one",
			arm.PickerType, parameters.HandlerPluginName, handler.arm.HandlerType,
			arm.PickerType, arm.HandlerType)
	}

	return &Picker{
		typedName:     fwkplugin.TypedName{Type: arm.PickerType, Name: name},
		handler:       handler,
		prefixDataKey: handler.prefixDataKey,
		tokenDataKey:  handler.tokenDataKey,
		metrics:       newPluginMetrics(name, arm.PickerType),
	}, nil
}
