...
// ScoredEndpoint.Score is 0, and the endpoints reach the picker in MAP ORDER.
//
// This is the case that would catch a random decode placement. runPickerPlugin builds its
// slice by ranging a map (scheduler_profile.go:199-205), itself seeded by ranging the
// Filter's output, so the Filter's ordering is discarded and Go's map iteration randomizes
// what arrives. Any dependence on arrival order shows up here as two identical requests
// landing on different pods.
//
// With several equally-priced H100 decode endpoints the tie must resolve the same way every
// time, and to the lowest ID -- which is what the ascending-ID enumeration in Policy.decide
// guarantees.
func TestFullPickerPathIsDeterministicUnderAnAllZeroScoreField(t *testing.T) {
	handler, picker := buildArm(t, armConfig())
	handler.policy.observeCompletedOutput(sloClassStandard, 300)

	// Several decode endpoints, so a random choice would show up quickly. Three share the
	// same GPU type and therefore tie exactly; the A100 is priced higher and must lose.
	endpoints := []fwksched.Endpoint{
	...
	if len(seen) != 1 {
		t.Fatalf("the arm placed decode on %d different endpoints across 60 identical requests "+
			"(%v); the input slice is map-ordered, so the argmin must impose its own order or "+
			"two byte-identical requests can route differently", len(seen), seen)
	}
	if _, ok := seen["decode-a"]; !ok {
		t.Errorf("expected the lowest-ID endpoint of the tied H100 group, got %v", seen)
	}
}

// newFakeHandleWithRegistry shares ONE registry across handles, which is what a real EPP
// does: handle.Metrics() is the process-wide registerer. contract_test.go uses it to build
// both arms against a single registry.
