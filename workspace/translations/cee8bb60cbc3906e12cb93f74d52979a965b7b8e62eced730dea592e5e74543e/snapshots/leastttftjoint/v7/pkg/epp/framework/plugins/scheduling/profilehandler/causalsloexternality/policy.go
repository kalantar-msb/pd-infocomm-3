...
// director.go:601-632, while the final chunk runs on the request goroutine at :539).
type Policy struct {
	cfg Config

	// objective is the per-candidate cost this instance minimises -- the ONE thing the
	// two registered arms differ in. It mirrors upstream's swappable costFn
	// (sim/edpp.go:1334 sets it per rule, :1453 for the least-TTFT branch). See arm.go.
	objective Objective
...
// newPolicy builds a Policy over a validated Config, running the given arm's objective.
func newPolicy(cfg Config, table *shadowTable, metrics *pluginMetrics, objective Objective) *Policy {
	return &Policy{
		cfg:         cfg,
		objective:   objective,
		table:       table,
...
// capacityLive reports whether the capacity account is both READ by this arm's
// objective and enabled by config.
//
// Both conditions are required and the objective one comes FIRST. Ablation.NoCapacity
// is a bool, so its zero value is false, which READS AS "capacity enabled" -- and an
// arm that carries no capacity term omits the whole ablation block from its plugin
// config. Testing config alone would therefore switch the account on for exactly the
// arm that has no use for it, and then demand capacity.tauRefUs at startup for a term
// nothing evaluates.
func (p *Policy) capacityLive() bool {
	return p.objective.ReadsSLOValueConfig() && !p.cfg.Ablation.NoCapacity
}
...
func (p *Policy) decide(ec *evalCtx, decodeSnaps, prefillSnaps []Snapshot, scorerDecodeID, scorerPrefillID string) (candidate, bool) {
	...
	// ENUMERATION ORDER IS THE OBJECTIVE'S, and it is one of the two things an arm is
	// allowed to differ in.
	//
	// For the focal arm the inherited scorer's decode pick is enumerated FIRST, so that
	// restricting the enumeration to it reproduces the decomposed rule exactly. AND THE
	// SAME APPLIES ON THE PREFILL SIDE: upstream reorders the prefill snapshots too
	// whenever a prefill scorer is injected (sim/edpp.go:1387-1390). Iterating plain
	// prefillSnaps instead changes which candidate wins an exact tie. With one prefill
	// endpoint the two orders coincide, which is exactly why this is easy to port wrong
	// and hard to notice: config.md section 2 deploys 1P, so a second prefill endpoint
	// would silently change tie resolution.
	//
	// The comparator arm declines the reordering, because upstream's least-ttft branch
	// iterates decodeSnaps directly (sim/edpp.go:1456) and applies scorerFirstSnapshots
	// only in the SLO-externality and causal-VaR branches. Both directions are wrong for
	// the other arm and both are wrong only on an EXACT tie, which is why this is a
	// declared property of the objective rather than a shared default.
	orderedD, orderedP := decodeSnaps, prefillSnaps
	if p.objective.ScorerFirstEnumeration() {
		orderedD = scorerFirstSnapshots(decodeSnaps, scorerDecodeID)
		orderedP = scorerFirstSnapshots(prefillSnaps, scorerPrefillID)
	}

	// The decomposition control depends on orderedD[0] being the inherited scorer's
	// pick, so it is reachable only for an arm that enumerates scorer-first. For any
	// other arm Ablation is absent from the plugin config and Decomposed is false, but
	// the objective test states the dependency rather than relying on that.
	if p.objective.ScorerFirstEnumeration() && p.cfg.Ablation.Decomposed && len(orderedD) > 1 {
	...
	// ONE candidate set, ONE scale, ONE argmin, for BOTH arms. The cost function is the
	// only thing that varies -- exactly as upstream, where decideJoint's costFn is set
	// per rule (sim/edpp.go:1453 for the least-TTFT branch) over this same cross
	// product. An arm that enumerated its own candidates would no longer be comparable
	// to the other, and nothing in a per-arm test could see the difference.
	eval := Eval{p: p, ec: ec}
	for _, ds := range orderedD {
		consider(candidate{dID: ds.ID, local: true, J: p.objective.Cost(eval, ds, nil)})
		for i := range orderedP {
			ps := orderedP[i]
			consider(candidate{dID: ds.ID, pID: ps.ID, local: false, J: p.objective.Cost(eval, ds, &ps)})
		}
	}
	if best == nil {
	...
	// same demand expression the candidate score used. bookCapacityWork already refuses
	// when ablation.noCapacity is set; the guard here is what additionally keeps an arm
	// whose objective carries no capacity term -- and which therefore omits the ablation
	// block entirely, leaving noCapacity at its false zero value -- from booking work
