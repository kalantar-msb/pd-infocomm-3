...
| `ablation.decomposed` | `bool` | Yes | Fix decode placement to the inherited pick, reproducing a decode-first decomposition. |
...
| `causal_slo_externality_tokenization_unavailable_total` | That the arm declined to decide and an inherited pick stood instead. |
...
| `causal_slo_externality_candidate_rejected_total` (by `reason`) | That a mislabelled endpoint left the candidate set -- otherwise indistinguishable from a routing preference. |
...
| `causal_slo_externality_placement_total` | What the argmin actually picked, local versus disaggregated. |
...
- **`S_pf` is an over-estimate.** What is being prefilled in the current engine step is not
  observable from the EPP, so the shadow sum is capped per occupant and in total. The
  residual over-estimate over-states local prefill inflation.
