...
func TestDecideEnumeratesTheFullCrossProduct(t *testing.T) {
	...
	// Every one of the six candidates must be reachable and priced.
	seen := map[string]float64{}
	for _, ds := range decodes {
		seen[ds.ID+"|local"] = candidateJ(t, p, inputLen, aps, ds, nil)
		for i := range prefills {
			ps := prefills[i]
			seen[ds.ID+"|"+ps.ID] = candidateJ(t, p, inputLen, aps, ds, &ps)
		}
	}
	if len(seen) != 6 {
		t.Fatalf("enumerated %d candidates, want 2 local + 4 disaggregated", len(seen))
	}
	...
	wantMin := math.Inf(1)
	var wantKey string
	for key, j := range seen {
		if j < wantMin {
			wantMin, wantKey = j, key
		}
	}
