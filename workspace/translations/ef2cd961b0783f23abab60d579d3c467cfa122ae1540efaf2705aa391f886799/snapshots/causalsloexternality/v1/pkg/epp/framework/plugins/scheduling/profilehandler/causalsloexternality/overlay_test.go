...
func loadOverlayPlugins(t *testing.T) pluginsConfig {
	t.Helper()
	raw, err := os.ReadFile(overlayPath(t))
	if err != nil {
...
// TestOverlayDeclaresBothHalvesInTheRequiredOrder pins the structural facts a reviewer would
// otherwise have to check by eye: the handler is declared before the picker, the picker
// references it by name, exactly one profile exists, and it names the picker and no scorers.
...
func TestOverlayDeclaresTheProducersTheArmBinds(t *testing.T) {
	...
	if n := cfg.Signals.PrefixMatchInfoProducerName; n != "" && !declared[n] {
		t.Errorf("signals.prefixMatchInfoProducerName names %q, which the overlay does not declare", n)
	}
	if n := cfg.Signals.TokenizedPromptProducerName; n != "" && !declared[n] {
		t.Errorf("signals.tokenizedPromptProducerName names %q, which the overlay does not declare", n)
	}
}
