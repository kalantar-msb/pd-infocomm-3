...
func testPolicy(t *testing.T, cfg Config) *Policy {
	...
	return newPolicy(cfg, newShadowTable(cfg.ShadowTable, int64(cfg.Engine.BlockSize), metrics), metrics, focalObjective{})
}
...
func TestDecideEnumeratesFullCrossProduct(t *testing.T) {
	...
	// The estimator-substitution counter is incremented once per admission estimate, so it
	// is a direct probe of how many candidates were scored.
	//
	// THIS PROBE IS VALID ONLY WHILE THE ROLLOUT GUARD IS CLOSED. estimateTAdm increments
	// unconditionally, and with SchedulerStateObserved permanently false every estimate it
	// counts is genuinely a substitution. If a future engine patch opens that guard, the
	// closed-form estimate is still computed and counted but then DISCARDED by the rollout
	// override in scoreCandidate -- so the counter would over-report substitutions and this
	// test would silently stop measuring candidate count. At that point either move the
	// increment to the point where the rollout actually declines, or count candidates
	// directly.
...
// TestDecideHonoursANonEmptyPreferenceDeterministically is the case the original
// determinism test could not see, because it passed an empty preference: with a real
// preference the enumeration order changes, and an exact tie must resolve to the preferred
// endpoint rather than to the lowest ID.
func TestDecideHonoursANonEmptyPreferenceDeterministically(t *testing.T) {
	residents := []RunningReqState{
		{RequestID: "r0", StepsDone: 10, SLOClass: sloClassStandard, TTFTSet: true,
			ArrivalUs: 99_000_000, FirstTokenUs: 99_100_000, KVBlocks: 50},
	}
	// Two IDENTICAL decode endpoints, so the winner is decided purely by enumeration order.
	decodes := []Snapshot{
		decodeSnap("dA", "H100_SXM_80GB", 4, 0, 20000, residents),
		decodeSnap("dB", "H100_SXM_80GB", 4, 0, 20000, residents),
	}
	...
	for _, preferred := range []string{"dA", "dB"} {
		var first candidate
		for i := 0; i < 20; i++ {
			p := testPolicy(t, focalConfig())
			p.observeCompletedOutput(sloClassStandard, 300)
			got, ok := p.decide(testEvalCtx(p, 4000, aps), decodes, prefills, preferred, "")
			if !ok {
				t.Fatal("decide must succeed")
			}
			if i == 0 {
			...
			if got.dID != first.dID || got.pID != first.pID || got.local != first.local {
				t.Fatalf("preference %q: non-deterministic decision %+v then %+v", preferred, first, got)
			}
		}
		if first.dID != preferred {
			t.Errorf("an exact tie must resolve to the preferred endpoint %q, got %q", preferred, first.dID)
		}
	}
}
