...
// THE METRIC NAMES IN THIS FILE ARE THE FOCAL ARM'S, INHERITED VERBATIM, AND THEY ARE A
// DELIBERATE MISNOMER HERE.
//
// Everything below is a byte-for-byte copy of ../causalsloexternality/metrics.go except this
// comment block and the package clause, under the verbatim-copy contract of config.md
// section 9.3. So this arm emits series named `causal_slo_externality_*` -- for example
// `causal_slo_externality_estimator_substitution_total` -- while having no externality term
// at all: no value kernels, no selfGood, no capacity account. A reader coming from Prometheus
// rather than from this source WILL find that surprising, so:
//
//   THE TWO ARMS ARE DISTINGUISHED BY THE plugin_name AND plugin_type LABELS, NOT BY THE
//   METRIC NAME. Every collector below carries both. This arm's series arrive with
//   plugin_type="least-ttft-joint-handler" or "least-ttft-joint-picker".
//
// WHY SHARED NAMES ARE THE RIGHT CHOICE, since the alternative is superficially tidier.
// config.md's D8 requires comparing the two arms' tokenization-decline RATES before any
// result is read, and D1 requires comparing their estimator regimes. In the shipped shape
// each arm is its own overlay and its own EPP, so those comparisons happen ACROSS RUNS.
// Identical family names plus differing plugin_type labels make each comparison a SINGLE
// query with a label selector. Renaming the families would split every one of them into two
// queries that then have to be joined by hand -- for no analytical gain, since the labels
// already separate the arms.
//
// THE COLLISION THAT SHARED NAMES WOULD CAUSE IS UNREACHABLE IN ANY VALID CONFIGURATION.
// registerMetrics runs at FACTORY time (HandlerFactory, PickerFactory), not at package init,
// so compiling both arms into one binary registers nothing. Instantiating both would collide
// -- but it cannot happen: both arms' handlers implement fwksched.ProfileHandler and
// buildSchedulerConfig permits exactly one across all instantiated plugins
// (configloader.go:275-284, "multiple profile handlers found (%s, %s); only one is allowed").
// Two arms in one EndpointPickerConfig is already invalid before metrics matter.
//
// ONE DEBUGGING TRAP, LEFT HERE ON PURPOSE. instantiatePlugins runs at
// configloader.go:147, BEFORE buildSchedulerConfig's uniqueness check at :160. So if someone
// does configure both arms together, the SECOND arm's factory fails first with a Prometheus
// duplicate-registration error, and that error MASKS the clear "multiple profile handlers
// found" diagnosis. If you are reading a metrics-registration failure at startup, check
// whether the config declares two ProfileHandlers before investigating anything in this file
// -- the metrics error is the symptom, not the fault.
//
// Note also that registerMetrics's error text below names the causal-slo-externality metric.
// That reads oddly from this package but is accurate: it is the name of the family being
// registered.
