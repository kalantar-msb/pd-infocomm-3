...
func HandlerFactory(name string, rawParameters *json.Decoder, handle fwkplugin.Handle) (fwkplugin.Plugin, error) {
	...
	table := newShadowTable(cfg.ShadowTable, metrics)
...
func (h *Handler) snapshotFor(id, gpuType string, metrics *fwkdl.Metrics, blockSize int64) Snapshot {
	...
	decode, prefill := h.table.residentsFor(id)
	// A prefill-pool endpoint's occupants are the requests deflected TO it, which are
	// indexed by prefillID rather than decodeID.
	if poolOccupants := h.table.prefillOccupantsFor(id); len(poolOccupants) > 0 {
		prefill = append(prefill, poolOccupants...)
	}
	...
	return Snapshot{
		...
		ResidentPrefillTokens: h.table.residentPrefillTokens(id),
