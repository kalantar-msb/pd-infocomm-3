...
// TestMetricNamesAreDistinctFromTheFocalArms pins the registry-collision guard.
//
// The focal arm's registerMetrics tolerates a repeat registration only of the IDENTICAL
// collector object, so same-named collectors declared in two packages would make one arm's
// factory return an error and fail EPP startup. Distinct family names are what make both
// arms registerable in one binary. See the note at the top of metrics.go for why this
// costs the D8 comparison one extra query rather than making it unavailable.
func TestMetricNamesAreDistinctFromTheFocalArms(t *testing.T) {
	raw, err := os.ReadFile(filepath.Join(focalDir(t), "metrics.go"))
	if err != nil {
		t.Fatalf("read the focal arm's metrics.go: %v", err)
	}
	focalSource := string(raw)
	mineRaw, err := os.ReadFile("metrics.go")
	if err != nil {
		t.Fatalf("read metrics.go: %v", err)
	}
	for _, name := range metricNames(t, string(mineRaw)) {
		if strings.Contains(focalSource, `"`+name+`"`) {
			t.Errorf("metric %q is also declared by the focal arm. Both arms register into one "+
				"registry, and a duplicate family name under a DIFFERENT collector object fails "+
				"registration and therefore fails EPP startup.", name)
		}
...
// TestBothArmsRegisterIntoOneRegistry is the end-to-end form of the metric-naming contract,
// and it tests the failure that would actually be observed: EPP refusing to start.
//
// Both arms' factories call their own registerMetrics with handle.Metrics(), which is one
// registry per EPP. Prometheus rejects a duplicate family name backed by a DIFFERENT
// collector object, and neither arm's registerMetrics swallows that -- each tolerates only a
// repeat of its own collector. So same-named families in the two packages would make whichever
// arm was constructed second return an error from its factory.
//
// Constructing BOTH arms' handlers against ONE registry is the direct check. It also
// exercises the reverse order, because registration order in a real binary depends on the
// plugin config.
func TestBothArmsRegisterIntoOneRegistry(t *testing.T) {
	for _, order := range []string{"comparator first", "focal first"} {
		t.Run(order, func(t *testing.T) {
			registry := prometheus.NewRegistry()

			buildComparator := func() error {
				handle := newFakeHandleWithRegistry(registry)
				plugin, err := HandlerFactory("comparator", decoderFor(t, armConfig()), handle)
				if err != nil {
					return err
				}
				t.Cleanup(plugin.(*Handler).table.stop)
				return nil
			}
			buildFocal := func() error {
				handle := newFakeHandleWithRegistry(registry)
				_, err := causalsloexternality.HandlerFactory("focal", decoderFor(t, focalArmConfig()), handle)
				return err
			}

			first, second := buildComparator, buildFocal
			if order == "focal first" {
				first, second = buildFocal, buildComparator
			}
			if err := first(); err != nil {
				t.Fatalf("%s: building the first arm failed: %v", order, err)
			}
			if err := second(); err != nil {
				t.Fatalf("%s: building the second arm against the SAME registry failed: %v\n\n"+
					"Both arms are registered in cmd/epp/runner and both call registerMetrics with "+
					"the one per-EPP registry. A duplicate metric family name under a different "+
					"collector object fails registration, which fails the factory, which fails EPP "+
					"startup. This arm's families must stay namespaced least_ttft_joint_*.", order, err)
			}
		})
	}
}
