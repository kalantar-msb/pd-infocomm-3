...
type Handler struct {
	...
	// arm carries the objective this instance runs and the pair of plugin type
	// strings that switch it on. It is what lets ONE Handler type serve both
	// registered arms so they cannot drift apart on the shared machinery -- see
	// arm.go. The picker checks it before binding, because a picker bound to the
	// other arm's handler would silently run the other arm's objective while every
	// log line and metric label named this one.
	arm Arm
...
// HandlerFactory builds the FOCAL arm's handler. It is NewHandlerFactory(FocalArm) --
// see arm.go for why the construction is parameterised by arm rather than fixed.
var HandlerFactory = NewHandlerFactory(FocalArm)

// newHandler builds the handler for one arm, validates the whole configuration, and
// starts the shadow table's TTL sweeper.
//
...
// It is also arm-aware in BOTH directions -- an objective that reads the SLO-value half
// of Config requires it, and one that does not requires its ABSENCE, so a tau triple
// tuned for an arm that never reads it fails at startup instead of tuning nothing.
func newHandler(arm Arm, name string, rawParameters *json.Decoder, handle fwkplugin.Handle) (fwkplugin.Plugin, error) {
	cfg := Config{}
	if rawParameters == nil {
		return nil, fmt.Errorf("%s requires parameters: there are deliberately no defaults for the "+
			...
			"invented coefficients", arm.HandlerType)
	}
	if err := rawParameters.Decode(&cfg); err != nil {
		return nil, fmt.Errorf("failed to parse parameters of the %s plugin: %w", arm.HandlerType, err)
	}
	if err := cfg.validate(arm.Objective); err != nil {
		return nil, fmt.Errorf("%s configuration is invalid: %w", arm.HandlerType, err)
	}
	if err := registerMetrics(handle.Metrics()); err != nil {
		return nil, err
	}

	metrics := newPluginMetrics(name, arm.HandlerType)
	...
	h := &Handler{
		typedName: fwkplugin.TypedName{Type: arm.HandlerType, Name: name},
		arm:       arm,
		cfg:       cfg,
		table:     table,
		metrics:   metrics,
	...
	h.policy = newPolicy(cfg, table, metrics, arm.Objective)
	return h, nil
}
