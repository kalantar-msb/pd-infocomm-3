...
import (
	...
	"github.com/llm-d/llm-d-router/pkg/epp/framework/plugins/scheduling/profilehandler/leastttftjoint"
...
func (r *Runner) registerInTreePlugins() {
	...
	// The INFOCOM 2027 joint causal-SLO-externality arm: TWO registrations that share
	// one Policy and one shadow table. ProfileHandler.Pick and Picker.Pick share a
	// method name with different signatures
	// (pkg/epp/framework/interface/scheduling/plugins.go:47, :77), so no single Go type
	// can satisfy both -- the handler carries ProfileHandler + PreRequest +
	// ResponseBodyProcessor and the picker carries Filter + Picker.
	//
	// THE PICKER IS REGISTERED WITH ITS PLUGIN DEPENDENCIES, and that is required rather
	// than tidy. It resolves the handler with handle.Plugin(name), and instantiatePlugins
	// constructs in TOPOLOGICAL order over the plugin DAG
	// (pkg/epp/config/loader/configloader.go:244-250), not in config order. Two plugins
	// with no declared dependency between them sit at in-degree 0 and their relative
	// order comes from ranging a Go map (pkg/epp/util/utils.go:42-46), so a plain
	// Register here builds the picker before the handler in roughly 5 starts out of 6 and
	// crashloops the EPP. PickerConfigParser exposes the `pluginRef`-tagged handler name,
	// so the DAG orders the handler first.
	//
	// BOTH ARMS ARE REGISTERED Beta, and that is a deployment decision rather than a
	// stability claim. This paragraph covers the comparator arm below as well, so there is
	// one statement of the reason rather than two.
	//
	// ValidatePluginStability refuses to start the EPP when any instantiated plugin is Alpha
	// and --allow-experimental-plugins is absent
	// (pkg/epp/framework/interface/plugin/registry.go:107-116), enforced on both startup
	// paths below at :453 and :1125, with the flag at pkg/epp/server/options.go:239.
	//
	// THAT FLAG IS EXPRESSIBLE from the scenario overlay these arms are deployed from:
	// router.epp.flags is read into $eppFlags at
	// config/charts/routerlib/templates/_deployment.yaml:8 and rendered as `--<key>=<value>`
	// args on the `- name: epp` container (:74) by the range at :116-120, documented at
	// config/charts/routerlib/values.yaml:29-32, and llm-d-benchmark's router-values
	// template passes the scenario's whole router block through to the chart. So Alpha is
	// not ruled out by the deployment path.
	//
	// It is ruled out by what it would COST. An Alpha registration makes every arm's
	// overlay carry a required extra field -- router.epp.flags.allow-experimental-plugins --
	// whose omission prevents the EPP from starting, and the failure lands only after the
	// image is built and pushed, because nothing in the build or test gate instantiates
	// plugins. Nothing but that startup gate reads the stability level, so Beta costs
	// nothing in behaviour and removes a per-arm overlay field that is easy to leave out.
	// It also matches every in-tree profile handler and picker these arms are modelled on
	// (:611-615). None of the four types carries an upstream compatibility promise.
	fwkplugin.Register(causalsloexternality.HandlerPluginType, fwkplugin.StabilityBeta, causalsloexternality.HandlerFactory)
	fwkplugin.RegisterWithPluginDependencies(causalsloexternality.PickerPluginType, fwkplugin.StabilityBeta,
		causalsloexternality.PickerFactory, causalsloexternality.PickerConfigParser)

	// The least-TTFT joint COMPARATOR arm of the same experiment (config.md section 9.3).
	// Two registrations again, and for the same structural reason.
	//
	// IT IS THE SAME Handler AND Picker as the arm above, constructed against a different
	// Objective, so the two arms share the candidate set, the estimators, the physics, and
	// the prefix reads and differ IN THE OBJECTIVE ONLY. That is what attributes a
	// measured difference to the mechanism rather than to the machinery, and it is why the
	// comparator's package contains no physics of its own.
	//
	// ONLY ONE OF THE FOUR TYPES ABOVE AND BELOW CAN BE INSTANTIATED AT A TIME on the
	// handler side: exactly one ProfileHandler is permitted across the whole config
	// (pkg/epp/config/loader/configloader.go:382-391). Registering both is what lets ONE
	// image serve both arms, selected by which types the treatment overlay names; it does
	// not let them run together, and a config naming both handlers fails to start.
	//
	// Beta for the reason stated once above, which covers both arms.
	//
	// The picker is registered with its plugin dependencies for the same construction-order
	// reason as the arm above, and the parser it is registered with is THIS arm's: it
	// carries this arm's type string in its error messages, and the handler it resolves is
	// checked to belong to this arm rather than merely to be a *Handler. Both arms are the
	// same Go type, so an unchecked binding would run the other arm's objective while every
	// log line and every plugin_type metric label reported this one.
	fwkplugin.Register(leastttftjoint.HandlerPluginType, fwkplugin.StabilityBeta, leastttftjoint.HandlerFactory)
	fwkplugin.RegisterWithPluginDependencies(leastttftjoint.PickerPluginType, fwkplugin.StabilityBeta,
		leastttftjoint.PickerFactory, leastttftjoint.PickerConfigParser)
